...
from mod.data_op.process_arr import convert_series_values, drop_nans_and_add_noise
...
def _check_array_and_params(arr, var_types, methods, params):
	"""检查数据值格式, 类型, 维数, 分箱方法以及数据值类型与分箱方法的匹配关系"""
	# 检查数据格式.
	try:
		N, D = arr.shape
		assert (N > 0) & (D > 1)
...
class MultivarBinning(object):
	...
	def do_joint_binning(self, methods: list, params: list) -> (np.ndarray, list):
		...
		:returns: hist: 高维分箱后箱子中的频数; edges: list of lists, 各维度上的标签
		"""
...
if __name__ == '__main__':
	...
	multi_binning = MultivarBinning(arr, var_types)
	hist, edges = multi_binning.do_joint_binning(methods, params)
