...
from src.data_binning import VALUE_TYPES_AVAILABLE, METHODS_AVAILABLE
from mod.data_op.process_arr import convert_series_values
...
class UnivarBinning(object):
	...
	def binning_bounds(self) -> list:
		"""变量分箱边界"""
		percentiles = self.stat_characters['percentiles']
	...
	def isometric_binning(self, bins: int) -> (list, list):
		...
		# 分箱, 数据只会在分箱边界内进行分箱.
		freq_ns, intervals = np.histogram(self.x, bins, range = self.binning_bounds)
		labels = intervals[1:]  # **以每个分箱区间的右边界为label
	...
	def label_binning(self) -> (list, list):
		...
		# 统计freq_ns.
		df = pd.DataFrame(self.x, columns = ['label'])
		df['index'] = df.index
		freq_counts = df.groupby('label').count()
		freq_counts = freq_counts.to_dict()['index']
		
		freq_ns = []
		for i in range(len(labels)):
			freq_ns.append(freq_counts[labels[i]])
		
		return freq_ns, labels
	
	def do_univar_binning(self, method: str, **params) -> (list, list):
		"""序列分箱算法, 将各分箱算法整理为一个通用方法方便调用"""
...
if __name__ == '__main__':
	...
	univar_binning = UnivarBinning(x, x_type)
	...
	for method in ['isometric', 'equifreq', 'quasi_chi2']:
		if method == 'isometric':
			freq_ns, labels = univar_binning.isometric_binning(**test_params[method])
		elif method == 'equifreq':
			freq_ns, labels = univar_binning.equifreq_binning(**test_params[method])
		elif method == 'quasi_chi2':
			freq_ns, labels = univar_binning.quasi_chi2_binning(**test_params[method])
		test_results[method] = {'freq_ns': freq_ns, 'labels': labels}

	# 测试通用分箱函数.
	for method in ['isometric', 'equifreq', 'quasi_chi2']:
		freq_ns, labels = univar_binning.do_univar_binning(method, **test_params[method])
	...
	x = np.array(data[col])
	univar_binning = UnivarBinning(x, x_type)
	freq_ns, labels = univar_binning.label_binning()
